#include "cwechatrefund.h"
#include "pile/cconnectormanager.h"
#include <unistd.h>
#include "manage/cmanage.h"

CWeChatRefund::CWeChatRefund(QObject *parent) : QObject(parent)
{
    init();
}

void CWeChatRefund::init()
{
...
*/
void CWeChatRefund::request(const QByteArray& data)
{
    mData = data;
    m_flowRet = false;

    QUrl url;
    url.setUrl(CApp::getInstance()->getSBaseUrl() + "wechatRefund");
    QNetworkRequest req;
    req.setUrl(url);
...
void CWeChatRefund::reqTimeout()
{
    if(m_flowRet == false)
    {
        ...
        // 退款超时，继续退款，直到成功为止
        CManage::getInstance()->sendWechatRefund(mData);
