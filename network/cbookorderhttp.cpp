#include "cbookorderhttp.h"

CBookOrderHttp::CBookOrderHttp(QObject *parent) : QObject(parent)
{
    init();
}

void CBookOrderHttp::init()
{
 ...
 * 全部流水号都从新平台的地址上获取
*/
void CBookOrderHttp::request(const QByteArray& data)
{
    this->connectorID = connectorID;
    this->cardNo = cardNo;
    QUrl url;
    url.setUrl( CApp::getInstance()->getSBaseUrl() + "createBookingOrder");
    ...
    qDebug() << QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss") << "send book order create msg....";
...
void CBookOrderHttp::reqTimeout()
{
    if(m_bRet == true)
    {
        m_reply->close();
...
void CBookOrderHttp::reqReply()
{
    m_bRet = true;
    QByteArray tdata = m_reply->readAll();

    qDebug() << "create book order ret " << tdata;
